...
const H = 36;
const HOURS = Array.from({length:18}, (*,i) => i+7);
const LATE = Array.from({length:6}, (*,i) => i+1);
...
.toast{margin-top:.6rem;padding:.45rem 1rem;border-radius:2rem;font-size:.76rem;font-weight:600;color:#fff;pointer-events:auto;animation:toastIn .3s ease;box-shadow:0 2px 12px rgba(0,0,0,.15)}
...
.refl-ta{flex:1;padding:.55rem .65rem;background:var(–bg);border:1px solid var(–bdr);border-radius:.5rem;color:var(–txt);font-size:.82rem;outline:none;font-family:var(–font);resize:none;min-height:2.8rem;line-height:1.5}
.refl-ta:focus{border-color:var(–acc)}
.refl-send{width:2.6rem;height:2.6rem;border:none;border-radius:.5rem;background:var(–acc);color:#fff;font-size:1rem;cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center}
.refl-send:disabled{opacity:.35;cursor:default}
.refl-vis{display:flex;align-items:center;gap:.3rem;margin-bottom:.5rem}
.refl-vis-btn{font-size:.68rem;padding:.2rem .5rem;border-radius:1rem;cursor:pointer;border:1px solid var(–bdr);background:var(–bg);color:var(–txt3);font-family:var(–font)}
.refl-vis-btn.on{background:var(–accBg);border-color:var(–accBdr);color:var(–acc)}
.refl-published{background:var(–bg);border-radius:.6rem;padding:.7rem;border-left:3px solid var(–acc)}
...
.refl-edit-btn{font-size:.68rem;padding:.15rem .45rem;border-radius:1rem;border:1px solid var(–bdr);background:var(–white);color:var(–txt2);cursor:pointer;font-family:var(–font);margin-left:auto}
...
.cdb{width:100%;padding:.6rem .75rem;text-align:left;background:var(–bg);border:1px solid var(–bdr);border-radius:.5rem;color:var(–txt);font-size:.84rem;cursor:pointer;font-family:var(–font);margin-bottom:.3rem;display:flex;align-items:center;justify-content:space-between}
...
.stab{flex:1;padding:.5rem;border:none;border-radius:.42rem;background:transparent;color:var(–txt3);font-size:.8rem;font-weight:600;cursor:pointer;font-family:var(–font)}
.stab.on{background:var(–white);color:var(–txt);box-shadow:var(–shadow)}
.shint{font-size:.7rem;color:var(–txt3);margin-top:.6rem;text-align:center;line-height:1.5}
...
.qtag{padding:.3rem .65rem;border-radius:2rem;border:1px solid var(–bdr);background:var(–bg);color:var(–txt2);font-size:.75rem;font-weight:500;cursor:pointer;font-family:var(–font);white-space:nowrap}
...
@media(min-width:640px){.member-tabs{display:none}.tl-col-header{display:block !important}}
@media(max-width:639px){.tl-col{display:none}.tl-col.active{display:block}.tl-col-header{display:none !important}}
`;
...
const[pmEnd,setPmEnd]=useState(8);
const[pmStartStr,setPmStartStr]=useState(“07:00”);
const[pmEndStr,setPmEndStr]=useState(“08:00”);
...
const dur=lastDuration||1;
setPmContent(””);setPmStart(startH);setPmEnd(startH+dur);
setPmStartStr(fmtT(startH));setPmEndStr(fmtT(startH+dur));
setPlanModal({memberId:mid});
};
const savePlan=()=>{
if(!pmContent.trim()||!planModal)return;
const{memberId,planId}=planModal;const md=getMemberDay(selDate,memberId);const content=pmContent.trim();
let plans;
...
const dur=pmEnd-pmStart;if(dur>0&&dur<=12)setLastDuration(dur);
};
...
const saveActual=()=>{if(!detailModal)return;const{memberId,plan}=detailModal;const md=getMemberDay(selDate,memberId);setMemberDay(selDate,memberId,{…md,plans:md.plans.map(p=>p.id===plan.id?{…p,actual:dmActual}:p)});setDetailModal(null)};
...
const copyFromDate=(fromDate)=>{const src=getMemberDay(fromDate,curUser);const dest=getMemberDay(selDate,curUser);const copied=src.plans.map(p=>({…p,id:genId(),done:false,actual:””}));setMemberDay(selDate,curUser,{…dest,plans:[…dest.plans,…copied]});setCopyModal(false)};

const myDay=useMemo(()=>getMemberDay(selDate,curUser),[data,selDate,curUser]);
const sendReflection=()=>{if(!reflDraft.trim())return;const md=getMemberDay(selDate,curUser);setMemberDay(selDate,curUser,{…md,reflection:{text:reflDraft.trim(),isPublic:reflPub,sent:true}});setReflEditing(false)};
...
const handleLogout=()=>{
if(!confirm(“确定退出登录？\n\n你的打卡数据不会丢失，随时可以用邀请码 + 昵称重新登录。”))return;
LC.del(“cfg”);LC.del(“data”);
setPhase(“setup”);setGroupCode(””);setMembers([]);setCurUser(””);setData({});
showToast(“已退出”,“ok”);
};
...
if(!loaded)return<div className="app"><style>{makeCSS(theme)}</style><div className="load-screen"><p>加载中…</p></div></div>;
if(loadError&&phase===“setup”)return(
<div className="app"><style>{makeCSS(theme)}</style><div className="load-screen">
<p>网络连接失败</p>
...
<p className="shint">如果你曾使用过这个邀请码，输入之前的昵称会自动恢复你的数据；新昵称则会创建新身份加入</p>
</>)}
</div></div>
</div>
);
...
```
  <div className="topbar">
    <h1>{groupName}</h1><span className="code">{groupCode}</span><div className="spacer"/>
  ...
  <div className="date-nav">
    ...
    {navDates.map(d=>(<button key={d} className={`day-btn ${d===TODAY?"today":""} ${d===selDate&&d!==TODAY?"sel":""}`} onClick={()=>setSelDate(d)}>{d.slice(5)}<span className="d">周{dayLabel(d)}</span></button>))}
    <button className="arr" onClick={()=>{const d=new Date(selDate);d.setDate(d.getDate()+1);setSelDate(dk(d))}}>›</button>
  ...
  <div style={{display:"flex",justifyContent:"center",padding:".3rem .7rem",background:"var(--card2)",borderBottom:"1px solid var(--bdr)",flexShrink:0}}>
    ...
    <div className="tl-wrap">
        ...
        <div className="tl-grid" style={{height:(totalH+1)*H*2}}>
          ...
          <div className="tl-cols">
            {sortedMembers.map((m,mi)=>{
              ...
              return(<div key={m.id} className={`tl-col ${selMember===m.id?"active":""}`}>
                ...
                <div className="tl-col-body" style={{height:(totalH+1)*H*2}}>
                  ...
                  {(()=>{
                    ...
                    plans.forEach(p=>{for(let c=active.length-1;c>=0;c--)if(active[c]&&active[c].end<=p.start)active[c]=null;
                      let placed=-1;for(let c=0;c<active.length;c++)if(!active[c]){active[c]=p;placed=c;break}
                      if(placed===-1){placed=active.length;active.push(p)}layout[p.id]={col:placed}});
                    plans.forEach(p=>{let maxCol=layout[p.id].col;
                    ...
                    return plans.map(p=>{
                      ...
                      const contentLines=height<50?1:height<90?2:height<140?3:height<200?5:999;
                      return(<div key={p.id} className={`plan-card ${isDone?"done":""} ${hasActual?"changed":""}`}
                        ...
                        <div className="pc-content" style={{WebkitLineClamp:contentLines}}>{p.content}</div>
                        {hasActual&&<div className="pc-actual">实际: {p.actual}</div>}
                        <div className={`pc-check ${isDone?"on":""}`} onClick={e=>{e.stopPropagation();if(m.id===curUser)toggleDone(m.id,p.id)}}>{isDone?"✓":""}</div>
                      </div>)})
                  })()}
      ...
      <div className="refl-section">
        ...
        {myDay.reflection.sent&&!reflEditing?(<div className="refl-published">
          <div className="refl-pub-text">{myDay.reflection.text}</div>
          <div className="refl-pub-meta"><span>{myDay.reflection.isPublic?"🔓 公开":"🔒 仅自己"}</span><button className="refl-edit-btn" onClick={startEditReflection}>编辑</button></div>
        </div>):(<>
        ...
        <div className="refl-others">
          {sortedMembers.filter(m=>m.id!==curUser).map(m=>{const md=getMemberDay(selDate,m.id);
  ...
  {view==="settings"&&(
    <div className="page">
      <div className="ss"><h3>✏️ 小组名称</h3><div className="sc">
        <input className="si" value={editGroupName} onChange={e=>setEditGroupName(e.target.value)} placeholder="给你的打卡组起个名字"/>
        <button className="bp" style={{width:"100%"}} onClick={async()=>{const n=editGroupName.trim()||"时间轴打卡";setGroupName(n);setEditGroupName(n);await saveCfg({groupName:n});showToast("✓ 组名已保存","ok")}}>保存组名</button>
      </div></div>

      <div className="ss"><h3>🎨 外观</h3><div className="sc">
        <div className="theme-toggle" onClick={toggleDark}><div className={`theme-track ${dark?"on":""}`}><div className="theme-knob"/></div><span className="theme-label">{dark?"暗色模式":"亮色模式"}</span></div>
      </div></div>

      <div className="ss"><h3>👥 成员</h3><div className="sc">
        <div className="ml">{members.map((m,i)=>(<div key={m.id} className="mi">
          <div className="md" style={{background:MCOLS[i%MCOLS.length]}}/>
          {editingMemberId===m.id?(<div style={{flex:1,display:"flex",gap:".3rem",alignItems:"center"}}>
            <input className="ar ii" style={{flex:1}} value={editingMemberName} onChange={e=>setEditingMemberName(e.target.value)} placeholder="输入新昵称" autoFocus onKeyDown={e=>{if(e.key==="Enter")renameMember(m.id,editingMemberName)}}/>
      ...
      <div className="ss"><h3>📤 导出</h3><div className="sc">
        ...
        <p style={{fontSize:".7rem",color:"var(--txt3)",marginTop:".3rem"}}>起始和结束选同一天可导出当天记录</p>
      </div></div>

      <div className="ss"><h3>🚪 退出</h3><div className="sc">
        <button className="bdel" style={{width:"100%"}} onClick={handleLogout}>退出登录</button>
        <p style={{fontSize:".72rem",color:"var(--txt3)",marginTop:".35rem",lineHeight:1.5}}>退出后打卡数据不会丢失，随时可以用邀请码 + 之前的昵称重新登录恢复</p>
      </div></div>
    </div>
  )}
  ...
  {planModal&&<div className="modal-bg" onClick={()=>setPlanModal(null)}><div className="modal" onClick={e=>e.stopPropagation()}>
    ...
    <div className="time-row">
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:".3rem"}}>
        <input className="inp" style={{marginBottom:0,fontFamily:"var(--mono)",textAlign:"center",fontSize:".9rem"}} value={pmStartStr} onChange={e=>{setPmStartStr(e.target.value);const v=parseTimeStr(e.target.value);if(v!==null)setPmStart(v)}} placeholder="07:00"/>
        <div style={{display:"flex",flexWrap:"wrap",gap:".2rem"}}>{[7,9,11,13,15,17,19,21].map(h=>(<button key={h} className="qtag" style={{padding:".15rem .4rem",fontSize:".65rem"}} onClick={()=>{setPmStart(h);setPmStartStr(fmtT(h))}}>{fmtH(h)}</button>))}</div>
      </div>
      <span style={{color:"var(--txt3)",flexShrink:0,padding:"0 .2rem"}}>至</span>
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:".3rem"}}>
        <input className="inp" style={{marginBottom:0,fontFamily:"var(--mono)",textAlign:"center",fontSize:".9rem"}} value={pmEndStr} onChange={e=>{setPmEndStr(e.target.value);const v=parseTimeStr(e.target.value);if(v!==null)setPmEnd(v)}} placeholder="08:00"/>
        <div style={{display:"flex",flexWrap:"wrap",gap:".2rem"}}>{[8,10,12,14,16,18,20,22].map(h=>(<button key={h} className="qtag" style={{padding:".15rem .4rem",fontSize:".65rem"}} onClick={()=>{setPmEnd(h);setPmEndStr(fmtT(h))}}>{fmtH(h)}</button>))}</div>
      </div>
    </div>
    <div style={{display:"flex",alignItems:"center",gap:".3rem",marginBottom:".55rem",flexWrap:"wrap"}}>
      <span style={{fontSize:".65rem",color:"var(--txt3)",fontWeight:500}}>时长</span>
      {[{label:"30m",dur:0.5},{label:"1h",dur:1},{label:"1.5h",dur:1.5},{label:"2h",dur:2},{label:"3h",dur:3}].map(({label,dur})=>{
        const isActive=Math.abs((pmEnd-pmStart)-dur)<0.01;
        return(<button key={label} className="qtag" style={{padding:".2rem .55rem",fontSize:".7rem",...(isActive?{background:"var(--accBg)",borderColor:"var(--accBdr)",color:"var(--acc)"}:{})}} onClick={()=>{const newEnd=pmStart+dur;setPmEnd(newEnd);setPmEndStr(fmtT(newEnd))}}>{label}</button>);
      })}
    </div>
    <label>计划内容</label>
    {recentInputs.length>0&&!planModal.planId&&(<div><div className="qtags-label">最近使用</div><div className="qtags">{recentInputs.map((r,i)=>(<button key={i} className="qtag" onClick={()=>setPmContent(r)}>{r}</button>))}</div></div>)}
  ...
  {detailModal&&<div className="modal-bg" onClick={()=>setDetailModal(null)}><div className="modal" onClick={e=>e.stopPropagation()}>
    ...
    <div className="chk-row" onClick={()=>{toggleDone(detailModal.memberId,detailModal.plan.id);setDetailModal({...detailModal,plan:{...detailModal.plan,done:!detailModal.plan.done}})}}>
      <div className={`chk-box ${detailModal.plan.done?"on":""}`}>{detailModal.plan.done?"✓":""}</div><span className="chk-label">已按预期完成</span>
    </div>
  ...
  {copyModal&&<div className="modal-bg" onClick={()=>setCopyModal(false)}><div className="modal" onClick={e=>e.stopPropagation()}>
    ...
    {datesWithPlans.length===0?<p style={{color:"var(--txt3)",textAlign:"center",padding:"1.2rem"}}>暂无历史计划</p>:(
      <div className="copy-dates">{datesWithPlans.map(d=>(<button key={d} className="cdb" onClick={()=>copyFromDate(d)}><span>{d} 周{dayLabel(d)}</span><span className="cc">{getMemberDay(d,curUser).plans.length} 项</span></button>))}</div>
    )}
